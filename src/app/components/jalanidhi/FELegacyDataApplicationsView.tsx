...
export default function FELegacyDataApplicationsView() {
  ...
  const fetchApplications = async () => {
    try {
      setLoading(true);
      console.log('[FE LEGACY APPS] Fetching legacy data applications for Field Engineer...');

      const response = await fetch(
        'https://' + projectId + '.supabase.co/functions/v1/make-server-698be164/legacy-data/applications?stage=field_engineer',
        {
          method: 'GET',
          headers: {
      ...
      if (!response.ok) {
        console.error('[FE LEGACY APPS] API Error:', response.statusText);
        setApplications([]);
        setLoading(false);
        return;
      }
      ...
      console.log('[FE LEGACY APPS] API Response:', data);
      ...
      if (data.success) {
        const transformed = (data.applications || []).map((app: any) => {
          ...
          } else if (app.status === 'fe_verified') {
        ...
        console.log('[FE LEGACY APPS] Loaded:', transformed.length);
      } else {
        console.error('[FE LEGACY APPS] Error:', data.error);
        setApplications([]);
      }
    } catch (error) {
      console.error('[FE LEGACY APPS] Fetch error:', error);
      setApplications([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleView = (appId: string) => {
    // Navigate to detailed legacy data view for FE
    const event = new CustomEvent('navigate', {
      detail: '/jalanidhi/field-engineer/tap-connection/legacy-data/view/' + appId,
    });
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-600 font-['Poppins',sans-serif] mt-1">
            Verify legacy data entries submitted by Caseworker
          </p>
        </div>
      ...
      <div className="bg-white rounded-[10px] border border-[#e5e7eb] shadow-[0px_4px_6px_-1px_rgba(0,0,0,0.1),0px_2px_4px_-2px_rgba(0,0,0,0.1)] overflow-hidden">
        ...
        {filteredApplications.length === 0 ? (
          <div className="px-6 py-12 text-center">
            ...
            <p className="text-gray-500 font-['Poppins',sans-serif]">No legacy data applications pending verification</p>
            <p className="text-sm text-gray-400 font-['Poppins',sans-serif] mt-1">Applications submitted by Caseworker will appear here</p>
          </div>
        ) : (
