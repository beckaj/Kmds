...
import CaseworkerReconnectionView from './CaseworkerReconnectionView';
...
export default function CaseworkerReconnectionDashboard() {
  const [applications, setApplications] = useState<ReconnectionApplication[]>(() => {
    const saved = localStorage.getItem('caseworker_reconnection_applications');
  ...
  const [selectedApplication, setSelectedApplication] = useState<ReconnectionApplication | null>(() => {
    const saved = localStorage.getItem('caseworker_reconnection_selectedApp');
    return saved ? JSON.parse(saved) : null;
  });
  
  const [searchQuery, setSearchQuery] = useState(() => {
    const saved = localStorage.getItem('caseworker_reconnection_searchQuery');
    return saved || '';
  });
  
  const [statusFilter, setStatusFilter] = useState(() => {
    const saved = localStorage.getItem('caseworker_reconnection_statusFilter');
    return saved || 'all';
  });
  ...
  useEffect(() => {
    if (applications.length > 0) {
      localStorage.setItem('caseworker_reconnection_applications', JSON.stringify(applications));
    }
  }, [applications]);
  ...
  useEffect(() => {
    if (selectedApplication) {
      localStorage.setItem('caseworker_reconnection_selectedApp', JSON.stringify(selectedApplication));
    } else {
      localStorage.removeItem('caseworker_reconnection_selectedApp');
    }
  }, [selectedApplication]);

  useEffect(() => {
    localStorage.setItem('caseworker_reconnection_searchQuery', searchQuery);
  }, [searchQuery]);

  useEffect(() => {
    localStorage.setItem('caseworker_reconnection_statusFilter', statusFilter);
  }, [statusFilter]);
  ...
  const fetchApplications = async () => {
    try {
      setLoading(true);
      console.log('[CASEWORKER RECONNECTION] Fetching reconnection applications...');
      ...
      for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
          console.log(`[CASEWORKER RECONNECTION] Attempt ${attempt}/${maxRetries}...`);
          ...
          console.log('[CASEWORKER RECONNECTION] API Response:', data);
          ...
          if (data.success) {
            ...
            console.log('[CASEWORKER RECONNECTION] Loaded reconnection applications:', reconnectionApps.length);
          } else {
            console.error('[CASEWORKER RECONNECTION] API Error:', data.error);
            setApplications([]);
          }
          return; // Success — exit retry loop
        } catch (err) {
          lastError = err;
          console.warn(`[CASEWORKER RECONNECTION] Attempt ${attempt} failed:`, err);
          if (attempt < maxRetries) {
            const delay = attempt * 2000;
            console.log(`[CASEWORKER RECONNECTION] Retrying in ${delay}ms...`);
      ...
      // All retries failed
      console.error('[CASEWORKER RECONNECTION] All retry attempts failed:', lastError);
      setApplications([]);
    } catch (error) {
      console.error('[CASEWORKER RECONNECTION] Error fetching applications:', error);
      setApplications([]);
    } finally {
      setLoading(false);
    }
  ...
  const getQueueLabel = (application: ReconnectionApplication) => {
    ...
    if (status === 'sentToPlumberForReconnection' || status === 'plumber_accepted_reconnection' || status === 'reconnection_work_submitted') {
      return { label: 'Plumber', color: 'bg-orange-100 text-orange-800 border-orange-200' };
    }
    if (status === 'reconnection_completed' || status === 'approved') {
          ...
          <div className="md:w-64">
            <div className="relative">
              ...
              >
                ...
                <option value="sentToPlumberForReconnection">Sent to Plumber</option>
                <option value="plumber_accepted_reconnection">Plumber Working</option>
                <option value="reconnection_work_submitted">Work Done</option>
          ...
          <div className="p-12 text-center">
            <p className="text-gray-600 font-['Poppins',sans-serif]">
              {searchQuery || statusFilter !== 'all' 
                ...
                : 'No reconnection applications submitted yet.'}
            </p>
          </div>
        ) : (
