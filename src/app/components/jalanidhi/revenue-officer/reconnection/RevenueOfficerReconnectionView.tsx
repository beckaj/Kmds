...
export default function RevenueOfficerReconnectionView({ applicationId }: RevenueOfficerReconnectionViewProps) {
  ...
  const loadApplicationData = async () => {
    try {
      setLoading(true);
      console.log('[RO RECONNECTION VIEW] Fetching application:', applicationId);
      ...
      if (!response.ok) {
        console.error('[RO RECONNECTION VIEW] API Error:', response.statusText);
        setLoading(false);
        return;
      }

      const data = await response.json();
      console.log('[RO RECONNECTION VIEW] API Response:', data);
      
      if (data.success && data.application) {
        setApplication(data.application);
        console.log('[RO RECONNECTION VIEW] Application loaded:', data.application);
        ...
        console.error('[RO RECONNECTION VIEW] Error:', data.error);
      }
    } catch (error) {
      console.error('[RO RECONNECTION VIEW] Error loading application:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleForward = async () => {
    ...
    setProcessing(true);
    try {
      console.log('[RO RECONNECTION] Forwarding reconnection application:', {
      ...
      if (data.success) {
        console.log('[RO RECONNECTION] Application forwarded successfully');
        ...
        console.error('[RO RECONNECTION] Error forwarding application:', data.error);
