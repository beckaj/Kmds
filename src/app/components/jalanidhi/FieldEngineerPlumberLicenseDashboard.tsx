...
import { ChevronLeft, Search, Filter, Eye, Clock, CheckCircle, User, MapPin, FileText, Wrench, Building2, UserCheck, MessageSquare, AlertTriangle, RotateCcw, Shield } from 'lucide-react';
...
// Read-only review view for a plumber license application (Field Engineer)
    ...
    <div className="min-h-screen bg-[#f5f5fa] px-8 py-6">
      ...
      <div className="mb-6">
        <h1 className="text-3xl font-bold text-[#1f3a5f] font-['Poppins',sans-serif] mb-2">
          Verify Plumber License Application
        </h1>
                ...
                <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 mt-4 flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-amber-700 font-['Poppins',sans-serif] mt-1">
                      The Commissioner has sent this application back for re-review. Please address the concerns below and re-forward with updated comments.
                    </p>
                  </div>
                </div>
              )}
            ...
            <div className="space-y-4">
              <div className="bg-green-50 border border-green-200 rounded-lg p-4 flex items-center gap-3">
                ...
                <p className="text-green-800 font-['Poppins',sans-serif] text-sm">
                  This application has been verified and forwarded to the Commissioner.
                </p>
              </div>
              {feComment && (
...
export default function FieldEngineerPlumberLicenseDashboard() {
  ...
  const fetchApplications = async () => {
    try {
      setLoading(true);
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-698be164/plumber-license/field-engineer/applications`,
      ...
      if (data && data.success) {
        ...
        console.error('[PLUMBER LICENSE FE] API Error:', data && data.error ? data.error : 'Unknown');
        setApplications([]);
      }
    } catch (error) {
      console.error('[PLUMBER LICENSE FE] Error fetching:', error);
      setApplications([]);
    } finally {
      setLoading(false);
    }
  ...
  const getStatusBadge = (app: PlumberLicenseApp) => {
    const status = app.status;
    // Check if this is a sent-back app from commissioner
    const isSentBack = status === 'sentToFieldEngineer' && app.workflow && app.workflow.commissioner && app.workflow.commissioner.decision === 'sent_back';
  ...
  const filteredApps = applications.filter(app => {
    ...
    if (statusFilter === 'sentBack') {
      // Special filter for sent-back apps
      const isSentBack = app.status === 'sentToFieldEngineer' && app.workflow && app.workflow.commissioner && app.workflow.commissioner.decision === 'sent_back';
      return matchesSearch && isSentBack;
    }
  ...
  return (
    <div className="min-h-screen bg-[#f5f5fa] px-8 py-6">
      <div className="mb-6">
        ...
        <p className="text-gray-600 font-['Poppins',sans-serif]">
          Verify and process plumber license registration applications
        </p>
      </div>
        ...
        <div className="flex flex-col md:flex-row gap-4">
          ...
          <div className="md:w-64 relative">
            ...
            >
              ...
              <option value="sentBack">Sent Back by Commissioner</option>
              ...
              <tbody className="divide-y divide-gray-200">
                {filteredApps.map((app, index) => (
                  <tr key={app.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-4 text-center">{getStatusBadge(app)}</td>
                    <td className="px-4 py-4 text-center">
                      {(() => {
                        const isSentBackApp = app.status === 'sentToFieldEngineer' && app.workflow && app.workflow.commissioner && app.workflow.commissioner.decision === 'sent_back';
                        if (isSentBackApp) {
                          return (
                            <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-['Poppins',sans-serif] font-medium bg-amber-100 text-amber-800 border border-amber-200">
                              Sent Back by Commissioner
                            </span>
                          );
                        }
                        ...
                        const q = queueMap[app.status] || queueMap['sentToFieldEngineer'];
                        return (
                    ...
                    <td className="px-4 py-4 text-center">
                      {(() => {
                        const isSentBackForReview = app.status === 'sentToFieldEngineer' && app.workflow && app.workflow.commissioner && app.workflow.commissioner.decision === 'sent_back';
                        return (
                          <button
                            onClick={() => setSelectedApp(app)}
                            className={`inline-flex items-center gap-1.5 px-4 py-2 rounded-lg font-['Poppins',sans-serif] font-medium text-sm transition-colors mx-auto ${
                              isSentBackForReview
